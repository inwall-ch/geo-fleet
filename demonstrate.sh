...
# or we can try to start it detached but monitoring output is hard.
# For this script, we'll assume it's running or just skip.
...
# Run simulation for a limited time or until user interrupt
# Using timeout to run for 30 seconds
timeout 30s ./vendor/bin/sail artisan fleet:simulate
